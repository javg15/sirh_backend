...
const Catestudiostipos = db.catestudiostipos;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catestudiostipos_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catestudiostipos_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catestudiostipos.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(catestudiostipos => {
            if (!catestudiostipos) {
                return res.status(404).send({ message: "Catestudiostipos Not found." });
            }

            res.status(200).send(catestudiostipos);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
