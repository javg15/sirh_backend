...
const Catsindicato = db.catsindicato;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catsindicato_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catsindicato_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catsindicato.findOne({
        ...
        .then(catsindicato => {
            if (!catsindicato) {
                return res.status(404).send({ message: "Catsindicato Not found." });
            }

            res.status(200).send(catsindicato);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
