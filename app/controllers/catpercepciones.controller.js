...
const Catpercepciones = db.catpercepciones;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catpercepciones_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catpercepciones_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catpercepciones.findOne({
        ...
        .then(catpercepciones => {
            if (!catpercepciones) {
                return res.status(404).send({ message: "Catpercepciones Not found." });
            }

            res.status(200).send(catpercepciones);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
...
exports.getCatalogoSegunAnio = async(req, res) => {
    ...
    let query = "select c.id,concat(anio, lpad(c.percepciones::text,2,0::text)) as text " +
        "from catpercepciones as c " +
    ...
    Catpercepciones.findOne({
        ...
        .then(catpercepciones => {
            if (!catpercepciones) {
                ...
                catpercepciones.update(req.body.dataPack).then((self) => {
                    // here self is your instance, but updated
                    res.status(200).send({ message: "success", id: self.id });
                });
            }
