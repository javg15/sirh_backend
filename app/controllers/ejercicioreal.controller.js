...
const Ejercicioreal = db.ejercicioreal;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_ejercicioreal_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_ejercicioreal_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Ejercicioreal.findOne({
        ...
        .then(ejercicioreal => {
            if (!ejercicioreal) {
                return res.status(404).send({ message: "Ejercicioreal Not found." });
            }

            res.status(200).send(ejercicioreal);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
