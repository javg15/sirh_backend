...
const Catestatusquincena = db.catestatusquincena;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catestatusquincena_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catestatusquincena_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catestatusquincena.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(catestatusquincena => {
            if (!catestatusquincena) {
                return res.status(404).send({ message: "Catestatusquincena Not found." });
            }

            res.status(200).send(catestatusquincena);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
