...
const Catbajamotivo = db.catbajamotivo;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catbajamotivo_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catbajamotivo_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catbajamotivo.findOne({
        ...
        .then(catbajamotivo => {
            if (!catbajamotivo) {
                return res.status(404).send({ message: "Catbajamotivo Not found." });
            }

            res.status(200).send(catbajamotivo);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
