...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_gruposclase_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
...
exports.getCatalogoConHorasDisponiblesSegunPlantel = async(req, res) => {
    ...
    let query = "select distinct gc.id,concat(gc.grupo ,'-',gc.tiposemestre) as text " +
        ...
        "   AND (cast(fn_horas_disponibles(h.id,:id_semestre,CASE WHEN h.id_cattiposemestre=62 THEN 0 ELSE :id_cattiposemestre END)->>'horasDisponibles' as integer)>0 OR COALESCE(:id_personalhoras,0)<>0) " +
