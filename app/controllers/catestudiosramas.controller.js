...
const Catestudiosramas = db.catestudiosramas;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catestudiosramas_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catestudiosramas_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catestudiosramas.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(catestudiosramas => {
            if (!catestudiosramas) {
                return res.status(404).send({ message: "Catestudiosramas Not found." });
            }

            res.status(200).send(catestudiosramas);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
