...
const Plantillasdocsprofesional = db.plantillasdocsprofesional;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        ...
        query = "SELECT * FROM s_plantillasdocsprofesional_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_plantillasdocsprofesional_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Plantillasdocsprofesional.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(plantillasdocsprofesional => {
            if (!plantillasdocsprofesional) {
                return res.status(404).send({ message: "Plantillasdocsprofesional Not found." });
            }

            res.status(200).send(plantillasdocsprofesional);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
...
exports.getRecordPersonal = async(req, res) => {
    ...
    Plantillasdocsprofesional.findOne({
        ...
        .then(plantillasdocsprofesional => {
            ...
            Personal.findOne({
                    where: {
                        id: plantillasdocsprofesional.id_personal
                    }
                })
                .then(personal => {
                    if (!personal) {
...
exports.getCatalogo = async(req, res) => {
    ...
    Plantillasdocsprofesional.findAll({
...
exports.getConsecutivo = async(req, res) => {
    ...
    Plantillasdocsprofesional.max('consecutivo', {
            where: {
                [Op.and]: [{
    ...
    Plantillasdocsprofesional.findOne({
        ...
        .then(plantillasdocsprofesional => {
            if (!plantillasdocsprofesional) {
                ...
                Plantillasdocsprofesional.create(
                    req.body.dataPack
                ).then((self) => {
                ...
                plantillasdocsprofesional.update(req.body.dataPack).then((self) => {
                    // here self is your instance, but updated
                    res.status(200).send({ message: "success", id: self.id });
                });
            }
