...
const Catlocalidades = db.catlocalidades;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catlocalidades_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catlocalidades_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catlocalidades.findOne({
        ...
        .then(catlocalidades => {
            if (!catlocalidades) {
                return res.status(404).send({ message: "Catlocalidades Not found." });
            }

            res.status(200).send(catlocalidades);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
...
exports.getCatalogo = async(req, res) => {
    ...
    Catlocalidades.findAll({
            attributes: ['id', 'descripcion'],
            where: {
                id_catmunicipios: req.body.id_municipio
            },
            order: [
                ['descripcion', 'ASC'],
            ]
