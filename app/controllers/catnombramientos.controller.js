...
const Catnombramientos = db.catnombramientos;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catnombramientos_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catnombramientos_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catnombramientos.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(catnombramientos => {
            if (!catnombramientos) {
                return res.status(404).send({ message: "Catnombramientos Not found." });
            }

            res.status(200).send(catnombramientos);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
