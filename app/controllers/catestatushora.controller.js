...
const Catestatushora = db.catestatushora;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catestatushora_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catestatushora_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catestatushora.findOne({
        ...
        .then(catestatushora => {
            if (!catestatushora) {
                return res.status(404).send({ message: "Catestatushora Not found." });
            }

            res.status(200).send(catestatushora);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
