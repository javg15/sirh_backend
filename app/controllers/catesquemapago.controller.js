...
const Catesquemapago = db.catesquemapago;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catesquemapago_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catesquemapago_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catesquemapago.findOne({
        ...
        .then(catesquemapago => {
            if (!catesquemapago) {
                return res.status(404).send({ message: "Catesquemapago Not found." });
            }

            res.status(200).send(catesquemapago);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
