...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catbancos_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catbancos_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catbancos.findOne({
        ...
        .then(catbancos => {
            if (!catbancos) {
                return res.status(404).send({ message: "Catbancos Not found." });
            }

            res.status(200).send(catbancos);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
