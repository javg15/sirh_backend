...
const Catfuncionprimaria = db.catfuncionprimaria;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catfuncionprimaria_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catfuncionprimaria_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catfuncionprimaria.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(catfuncionprimaria => {
            if (!catfuncionprimaria) {
                return res.status(404).send({ message: "Catfuncionprimaria Not found." });
            }

            res.status(200).send(catfuncionprimaria);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
