...
const Catfuncionsecundaria = db.catfuncionsecundaria;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catfuncionsecundaria_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catfuncionsecundaria_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catfuncionsecundaria.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(catfuncionsecundaria => {
            if (!catfuncionsecundaria) {
                return res.status(404).send({ message: "Catfuncionsecundaria Not found." });
            }

            res.status(200).send(catfuncionsecundaria);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
