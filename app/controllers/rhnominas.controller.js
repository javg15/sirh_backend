...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_rhnominas_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
...
exports.getRecord = async(req, res) => {
    ...
    Rhnominas.findOne({
        ...
        .then(rhnominas => {
            if (!rhnominas) {
                return res.status(404).send({ message: "Rhnominas Not found." });
            }

            res.status(200).send(rhnominas);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
