...
const Catmunicipios = db.catmunicipios;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_catmunicipios_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_catmunicipios_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Catmunicipios.findOne({
        ...
        .then(catmunicipios => {
            if (!catmunicipios) {
                return res.status(404).send({ message: "Catmunicipios Not found." });
            }

            res.status(200).send(catmunicipios);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
...
exports.getCatalogo = async(req, res) => {
    ...
    Catmunicipios.findAll({
            attributes: ['id', 'descripcion'],
            where: {
                id_catregiones: (req.body.id_region == 0 ? {
                    [Op.gt]: 0
                } : req.body.id_region)
            },
            order: [
...
exports.getCatalogoSegunEntidad = async(req, res) => {

    Catmunicipios.findAll({
            attributes: ['id', 'descripcion'],
            where: {
                id_catestados: (req.body.id_region == 0 ? {
                    [Op.gt]: 0
                } : req.body.id_catestados)
            },
            order: [
                ['descripcion', 'ASC'],
            ]
