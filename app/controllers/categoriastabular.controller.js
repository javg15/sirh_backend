...
const Categoriastabular = db.categoriastabular;
...
exports.getAdmin = async(req, res) => {
    ...
    if (req.body.solocabeceras == 1) {
        query = "SELECT * FROM s_categoriastabular_mgr('&modo=10&id_usuario=:id_usuario')"; //el modo no existe, solo es para obtener un registro
        ...
        query = "SELECT * FROM s_categoriastabular_mgr('" +
...
exports.getRecord = async(req, res) => {
    ...
    Categoriastabular.findOne({
            where: {
                id: req.body.id
            }
        })
        .then(categoriastabular => {
            if (!categoriastabular) {
                return res.status(404).send({ message: "Categoriastabular Not found." });
            }

            res.status(200).send(categoriastabular);
        })
        .catch(err => {
            res.status(500).send({ message: err.message });
        });
...
exports.setRecord = async(req, res) => {
    Object.keys(req.body.dataPack).forEach(function(key) {
        if (key.indexOf("id_", 0) >= 0 || key.indexOf("cantidad", 0) >= 0) {
    ...
    Categoriastabular.findOne({
        ...
        .then(categoriastabular => {
            if (!categoriastabular) {
                ...
                categoriastabular.update(req.body.dataPack).then((self) => {
                    // here self is your instance, but updated
                    res.status(200).send({ message: "success", id: self.id });
                });
            }
